...
from methods.fuse import fuse_pca, unfuse_pca
...
for quality_level in tqdm(jpeg_quality_levels, total = len(jpeg_quality_levels)):
    ...
    with FileReader(data_path) as fr:
        for i,frame in enumerate(fr):
            ...
            ###
            ### FUSE
            packed, pca_info = fuse_pca(np.concatenate([rgb_image, colorized], axis=-1))
            ...
            ### FUSE
            unfused = unfuse_pca(packed_decompressed, pca_info)
            unfused_blured = unfuse_pca(packed_blured_decompressed, pca_info)
            rgb_rec, colorized_decompressed = unfused[:,:,:3], unfused[:,:,3:]
            rgb_blured_rec, colorized_blured_decompressed = unfused_blured[:,:,:3], unfused_blured[:,:,3:]
            ###
            ###RESTORE
...
csv_name = f"output_data/yolo_fuse_pca/{method}_{kwargs_hash}.csv"
